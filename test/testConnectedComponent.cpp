...
// desk2 dataset is large (2.9k+ shots) so images were sampled 1:30
TEST(testConnectedComponent, posesVisualizationDesk2) {

    for (int iterations = 0; iterations < 1; ++iterations) {
        int numberOfPosesInDataset = 75;
        int samplingRate = 30;
        double minCoefficientOfBiggestComponent = 0.4;
        std::string numberOfPosesString = std::to_string(numberOfPosesInDataset);
        std::string samplingRateString = std::to_string(samplingRate);
        double coefficientR = 1.8;
        double coefficientT = 1.8;

        std::string shortDatasetName = "desk2_";
        std::string datasetName = shortDatasetName + "sampled_" + numberOfPosesString + "_" + samplingRateString;
...
TEST(testConnectedComponent, posesVisualizationDesk) {
