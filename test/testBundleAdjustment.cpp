...
TEST(testBundleAdjustment, posesBundleAdjustmentUsingDepthBetterThanIRLS_largeNoLoop) {
    ...
    gdr::GTT::prepareDataset("/home/leoneed/Desktop/large_no_loop_fr2", "/home/leoneed/testGDR1/GDR/data/large_no_loop_fr2_3356_10", sampledIndices, "");
    double coefficientR = 1.8;
    double coefficientT = 1.8;
    gdr::CorrespondenceGraph correspondenceGraph("../../data/large_no_loop_fr2_3356_10/rgb",
                                                 "../../data/large_no_loop_fr2_3356_10/depth",
                                                 520.9, 325.1,
                                                 521.0, 249.7);
    correspondenceGraph.computeRelativePoses();
    ...
    std::string absolutePoses = "../../data/large_no_loop_fr2_3356_10/groundtruth_new.txt";
    std::vector<gdr::poseInfo> posesInfo = gdr::GTT::getPoseInfoTimeTranslationOrientation(absolutePoses);
    ...
    {
        // print poses IRLS to file
        std::string outputName = "/home/leoneed/Desktop/evaluate_ate_scale/large/IRLS.txt";
        std::ofstream computedPoses(outputName);
        for (int i = 0; i < posesInfo.size(); ++i) {
            Sophus::SE3d poseSE3 = posesInfo[0].getSophusPose() * posesIRLS[i];
    ...
    {
        // print poses BA depth to file
        std::string outputName = "/home/leoneed/Desktop/evaluate_ate_scale/large/BA.txt";
        std::ofstream computedPoses(outputName);
        for (int i = 0; i < posesInfo.size(); ++i) {
            Sophus::SE3d poseSE3 = posesInfo[0].getSophusPose() * bundleAdjustedPoses[i];
...
//    assert(posesGT.size() == 151);
    ...
    std::cout << "__________IRLS test report LARGE poses_____________" << std::endl;
    std::cout << "mean error translation: " << meanErrorT_IRLS_L2 << std::endl;
    std::cout << "mean error rotation: " << meanErrorR_IRLS_angDist << std::endl;
    std::cout << "__________BA test report LARGE poses_____________" << std::endl;
    std::cout << "mean error translation: " << meanErrorT_BA_L2 << std::endl;
    std::cout << "mean error rotation: " << meanErrorR_BA_angDist << std::endl;
